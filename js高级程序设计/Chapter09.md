...
`					safariVersion = 1;
				...
				browser.safari = bowser.ver = safariVersion;
			}
			...
			engine.ver = RegExp["$1"];
			enginegecko = parseFloat(engine.ver);
		...
		// 检测Android版本
		if(/Android (\d+\.\d+)/.test(ua)) {
			system.andorid = parseFloat(RegExp.$1);
		}
...
- [ ] 不能准确地使用能力检测或怪癖检测。
- [ ] 同一款浏览器在不同平台下具有不同的能力。（此时有必要确定浏览器位于哪个平台下。）
- [ ] 为了跟踪分析等目的需要知道确切的浏览器

### 总结

在使用客户端检测方法时，优先考虑**能力检测**，其次选择**怪癖检测**，最后考虑**用户代理检测**。
